...
class ReservationService(
    ...
    fun createReservation(userId: UUID, showId: UUID, request: ReservationReqDto): ReservationResDto {
        val user =
            userRepository.findById(userId).orElseThrow { CustomException(ExceptionResponseStatus.USER_NOT_FOUND) }
        val show =
        ...
        if (lockId == null) {
            throw CustomException(ExceptionResponseStatus.DUPLICATE_RESERVATION) // Lock 획득 실패 시 중복 예약으로 간주
        }
